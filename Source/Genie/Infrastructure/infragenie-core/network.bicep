...
resource virtualNetwork 'Microsoft.Network/virtualNetworks@2022-01-01' = if (empty(existingNetworkId)) {
...
module privateDnsZone 'private-dns-zone-only.bicep' = [for dnsZoneName in dnsZones: if (shouldCreateDnsZones && empty(dnsZoneResourceGroupId) && !empty(dnsZonesToLaydown) && (contains(dnsZonesToLaydown, 'All'))) { 
  name: dnsZoneName
  dependsOn: [
    switchVnets
    virtualNetwork
