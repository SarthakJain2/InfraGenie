...
$root = "Golden"
$files = @("$root/.tags/$Domain/$Domain.json", "$root/.tags/$Domain/$Domain-$Environment.json")
if ( $ProjectName -ne '') {
    $files += @("$root/.tags/$Domain/$Domain-$ProjectName.json", "$root/.tags/$Domain/$Domain-$ProjectName-$Environment.json")
}
