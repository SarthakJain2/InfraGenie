...
$settings = .\Golden\Scripts\LocalDx\Get-AppSettings.ps1 -Domain $Domain -Environment $Environment -ProjectName $ProjectName
...
if ( $SubscriptionId -eq "") {
    ...
    [string] $subscriptionName = .\Golden\Scripts\LocalDx\Get-SubscriptionName.ps1 -Domain $Domain -Environment $Environment -ProjectName $ProjectName
...
if ($ImageReferenceId -eq "") {
    ...
    if ( $paramTags.parameters.imageReferenceId.value -eq "") {       
        ...
        Push-Location
        Set-Location "./Golden/IaC-Templates/vm-scaleset-image"
