...
var subnets = [for subnet in subnetAddressPrefixes: {
  ...
  properties: {
    ...
    networkSecurityGroup: (contains(subnet, 'nsgId') && empty(subnet.nsgId)) ? null : {
      id: subnet.name == 'APIManagementSubnet' ? ((contains(subnet, 'nsgId') && (!empty(subnet.nsgId))) ? subnet.nsgId : apimNsgId) : subnet.name == 'HDinsight' ? ((contains(subnet, 'nsgId') && (!empty(subnet.nsgId))) ? subnet.nsgId : HDinsightNsgId) :((contains(subnet, 'nsgId') && (!empty(subnet.nsgId))) ? subnet.nsgId : nsgId)
    }
...
@batchSize(1)
resource subnet 'Microsoft.Network/virtualNetworks/subnets@2022-01-01' = [for subnet in subnets: if (empty(existingNetworkId) && shouldCreateSubnets) {
  name: subnet.name
...
module switchVnets './vnet-switcher.bicep' = if (!shouldAllowPublicAccess) {
  ...
  params: {
    ...
    subnetName: empty(existingNetworkId) ? subnets[0].name : existingSubnets[0]
